...
extern GLfloat gvtx[], gtxq[];
extern int cardw, cardh;
...
void renderpluggrabberbg( void )
{
  ...
  set_quad_tristrip( vtx, 3200-fgx, pgr_watery+16-fgy, 3328-fgx, pgr_watery+48-fgy );
  set_texc_tristrip( txq, 176.0f/256.0f, 192.0f/256.0f, 192.0f/256.0f, 208.0f/256.0f );
...
void renderpluggrabber( void )
{
  ...
  if( ch > 9.0f )
  {
    ...
    glColor4ub(255, 255, 255, 255);
    set_quad_tristrip( vtx, -sw, -ch, sw, ch );
...
void rendersludgemonster( void )
{
  ...
  if( smonsyadd != 0 )
  {
    render_sprite( &sprt2[((frame>>3)&1)+24], smonsxp-fgx, smonsyp-fgy, !smonsreturning, 0.0f );
  } else {
    render_sprite( &sprt2[25], smonsxp-fgx, smonsyp+(sin(smonsbob)*5.0f)-fgy, !smonsreturning, 0.0f );
  }
}
...
void renderfactory( void )
{
  ...
  for( y=0; y<9; y++, yo+=16 )
  {
    ...
    {
      tx = &bt[facmap[y*13+x]];
      set_quad_tri( &gvtx[o], xo, yo, xo+16, yo+16 );
...
void renderstargate( void )
{
  ...
  for( y=0; y<WOBPTS; y++ )
  {
    ...
    for( x=0; x<WOBPTS; x++ )
    {
      gtxq[o]   = tx+wx[y*WBW+x];
      gvtx[o++] = px;
      gtxq[o]   = ty+wy[y*WBW+x];
      gvtx[o++] = py;

      gtxq[o]   = tx+wx[y*WBW+x+1]+txa;
      gvtx[o++] = px+pxa;
      gtxq[o]   = ty+wy[y*WBW+x+1];
      gvtx[o++] = py;

      gtxq[o]   = tx+wx[(y+1)*WBW+x];
      gvtx[o++] = px;
      gtxq[o]   = ty+wy[(y+1)*WBW+x]+tya;
      gvtx[o++] = py+pya;
      ...
      gtxq[o]   = tx+wx[(y+1)*WBW+x+1]+txa;
      gvtx[o++] = px+pxa;
      gtxq[o]   = ty+wy[(y+1)*WBW+x+1]+tya;
      gvtx[o++] = py+pya;
...
//    render_sprite_tl( &psprt5[15], 64, 64, FALSE );
  ...
  if( rotated )
  {
    glTranslatef(x+72/2, y+56/2, 0.0f);
    glRotatef(270.0f, 0.0f, 0.0f, 1.0f);
    set_quad_tristrip( vtx, -56/2, -72/2, 56/2, 72/2 );
    set_texc_tristrip( txq, 0.0f, (84.0f*(float)cardh)/(256.0f*640.0f), (64.0f*(float)cardw)/(256.0f*480.0f), 0.0f );
  } else {
    set_quad_tristrip( vtx, x, y, x+72, y+56 );
#ifndef __android__
    set_texc_tristrip( txq, 0.0f, (64.0f*(float)cardw)/(256.0f*640.0f), (84.0f*(float)cardh)/(256.0f*480.0f), 0.0f );
#else
    set_texc_tristrip( txq, 0.0f, 1, 1, 0.0f );
    //set_texc_tristrip( txq, 0.0f, (64.0f*(float)cardw)/(256.0f*640.0f), (84.0f*(float)cardh)/(256.0f*480.0f), 0.0f );
#endif
  }
  glVertexPointer( 2, GL_FLOAT, 0, vtx );
...
*/
  ...
  if( rotated )
  {
    wl = (((gid.py-fgy)-24)*cardw)/480;
    wt = (((gid.px-fgx)+84)*cardh)/640;
    glCopyTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, wl, wt, (64*2*cardw)/480, (84*2*cardh)/640 );

  } else {

#ifdef __android__
	wl = (((gid.px-fgx)+84)*cardw)/640;
	wt = (((gid.py-fgy)-24)*cardh)/480;
	glCopyTexImage2D(GL_TEXTURE_2D,0,GL_RGB, wl, wt,(84*2*cardw)/640, (64*2*cardh)/480,0 );
#else
    wl = (((gid.px-fgx)+84)*cardw)/640;
    wt = (((gid.py-fgy)-24)*cardh)/480;
    glCopyTexSubImage2D( GL_TEXTURE_2D, 0, 0, 0, wl, wt, (84*2*cardw)/640, (64*2*cardh)/480 );
#endif
  }
}
/*
...
void rendersprinkler( void )
{
  if( strig[ST1_HOSE_PLACED] )
  {
    ...
    for( i=0; i<NUMDROPLETS; i++ )
    {
      ...
      set_quad_tri( &gvtx[o], x-sw, y-sw, x+sw, y+sh );
      set_texc_tri( &gtxq[o], tx->x, tx->y, tx->x+tx->w, tx->y+tx->h );
      o += 12;
    }
    glVertexPointer( 2, GL_FLOAT, 0, gvtx );
